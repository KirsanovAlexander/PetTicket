...
-- Partial index: воркер сканирует только pending-записи, готовые к отправке.
-- sent/failed/processing записи никогда не попадают в этот индекс — со
-- временем их станет на порядки больше, чем pending, и полный индекс по
-- всей таблице был бы избыточен.
